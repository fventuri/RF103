...
int main(int argc, char **argv)
{
  ...
  /* just playin' around with the tuner gain settings */
  int lna_gain = 4;
  int mixer_gain = 5;
  int vga_gain = 3;
  ...
  if (rf103_set_rf_mode(rf103, VHF_MODE) < 0) {
    fprintf(stderr, "ERROR - rf103_set_rf_mode() failed\n");
    goto DONE;
  }

  if (rf103_set_vhf_frequency(rf103, vhf_frequency) < 0) {
    fprintf(stderr, "ERROR - rf103_set_vhf_frequency() failed\n");
    goto DONE;
  }

  if (rf103_set_vhf_lna_gain(rf103, lna_gain) < 0) {
    fprintf(stderr, "ERROR - rf103_set_vhf_lna_gain() failed\n");
    goto DONE;
  }

  if (rf103_set_vhf_mixer_gain(rf103, mixer_gain) < 0) {
    fprintf(stderr, "ERROR - rf103_set_vhf_mixer_gain() failed\n");
    goto DONE;
  }

  if (rf103_set_vhf_vga_gain(rf103, vga_gain) < 0) {
    fprintf(stderr, "ERROR - rf103_set_vhf_vga_gain() failed\n");
    goto DONE;
  }
